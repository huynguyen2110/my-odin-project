...
export class Hashset {
    ...
    set(key) {
        ...
        if (linkedList.headNode == null || !linkedList.replaceValue(key)) {
            if(this.isResize()) {
                this.resize()
            }
            linkedList.prepend(key, null);
        }
    ...
    get(key) {
        ...
        if (linkedList.headNode == null) {
            ...
            return linkedList.find(key, key);
        }
    }
    ...
    resize() {
        ...
        for (let i = 0; i < allEntries.length; i++) {
            const hashIndex = this.#hash(allEntries[i]);
            let linkedList = this.bucket[hashIndex];
            linkedList.prepend(allEntries[i], null);
        }
    }
